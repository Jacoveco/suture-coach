...
**Verified closing the app's window (not just the stop script) actually stops the server.** Traced the real process chain `start-suture-coach.bat` creates: the `cmd /k` window → `npm run dev:real` → `cross-env` → `next dev` → the actual Node.js server — six processes deep. Discovered along the way that this machine's console windows (Windows Terminal-hosted) have no classic Win32 window handle, so `taskkill` without `/F` (which sends `WM_CLOSE`, the mechanism a literal X-button click uses) doesn't apply here; dug further and found `CTRL_CLOSE_EVENT` — the actual signal a window close sends — can only ever be generated by Windows itself for a real close/logoff/shutdown, never synthetically by another process (a documented Win32 restriction). Used the closest event Windows *does* allow generating programmatically, `CTRL_BREAK_EVENT`, which broadcasts to every process sharing that console via the same delivery mechanism — confirmed all six processes in the chain terminated together and port 3000 was freed, leaving only the now-empty outer shell (which a real window close destroys too, since the window itself is gone). Net result: closing the window is confirmed to reliably stop the app, for the reason above.
